...
- Integrity and reuse use hashes/identifiers derived from actual material inputs and artifacts, not manually maintained candidate-policy versions or configurable fingerprint-input lists. New Step 26 production also requires the approved D13-C3 journal-bearing checkpoint format before effects; matching hashes cannot waive format rejection. Historical snapshot validation uses compatible historical code/schema without modifying evidence or claiming new-production execution.
