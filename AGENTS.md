...
- Integrity and reuse use hashes/identifiers derived from actual material inputs and artifacts, not manually maintained candidate-policy versions or configurable fingerprint-input lists. New Step 26 production also requires the approved D13-C3 journal-bearing checkpoint format before effects; matching hashes cannot waive format rejection. Under Sections 1.6/D12.1.5/3.3.9, the current evaluator rejects unsupported historical formats without modifying evidence or claiming new-production execution.
