"""Deterministic identity and hard admissibility for LP endpoint pairs.

The pair filter applies validated participation, statement-type, and developmental
coordinate policy to explicitly supplied endpoints. Admissibility permits later
candidate consideration; it does not nominate a pair, attach evidence, or publish a
relationship.
"""
...
class LPCandidateFilter:
    ...
    @property
    def admissible_statement_type_pairs(self) -> tuple[tuple[str, str], ...]:
        """Return canonical type cohorts permitted by at least one relationship.

        Returns
        -------
        tuple[tuple[str, str], ...]
            Sorted unordered type pairs used to keep evidence nomination inside the
            configured closed-world participation boundary. Direction and coordinate
            permissions remain the responsibility of ``filter_pair``.
        """

        type_pairs: set[tuple[str, str]] = set()

        for source_type, target_type in self._builds_towards_type_pairs:
            type_pairs.add(
        ...
        for relates_to_type_pair in self._relates_to_type_pairs:
            statement_types = tuple(sorted(relates_to_type_pair))
            type_pairs.add((statement_types[0], statement_types[-1]))

        return tuple(sorted(type_pairs))
    ...
    def _admissible_decisions(
        ...
        if decisions:
            # Include the possibility of a later negative or ambiguous judgment.
            decisions.extend(
                (
    ...
    @property
    def eligible_sfis(self) -> tuple[LPSFIEligibility, ...]:
        """Return canonical copies of endpoints eligible for at least one relation.

        Returns
        -------
        tuple[LPSFIEligibility, ...]
            UUID-ordered participation records available to evidence nomination.
        """

        return tuple(
            record.model_copy(deep=True)
            for _, record in sorted(
                self._sfi_by_uuid.items(), key=lambda item: str(item[0])
            )
