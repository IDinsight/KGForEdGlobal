...
6. **LOCAL CODES (optional)**:
   - If you see an explicit section/table code tightly associated with a block/table, put
     it in `local_code` verbatim (punctuation matters).
...
Omit these fields (Python fills them after extraction): boundary_state, doc_key, pdf_name,
page_index, dpi, image_width, image_height, coord_space.
...
5. **NO FULL-PAGE BBOXES** unless the page contains exactly one non-table visual (diagram/
   illustration) with no readable body text. Such pages are rare.
...
Valid block_type values: {allowed_block_types}
  ...
  - "{BlockType.FOOTNOTE.value}": Bottom-of-page footnotes (not page numbers). Must be
...
2. Text-bearing blocks (heading/paragraph/caption/footnote/artifact): non-empty `text`
   required; `list_items` and `figure` must be null.
...
2. Use ISO 639-1 codes where available (e.g., 'fr', 'wo', 'sw'); use ISO 639-3 for
   languages without two-letter codes; "und" if unknown; "mul" if multiple languages appear
   in the SAME TextUnit.
...
2. **n_cols**: Set if the column count is clearly inferable. If set, at least one row's cells
   (counting col_spans) must add up to n_cols, and no row may exceed it. Omit if unsure.
3. **MERGED CELLS**: Use row_span/col_span only for clearly visible merges. If unsure, use
   1/1. Spanned cells (row_span>1 or col_span>1) must have non-null text. Do not duplicate
   merged-cell text into covered cells.
4. **ROW BOUNDARIES**: Every row must have at least one cell. Rows may be separated by rules
   or by consistent spacing. Numbered/bulleted lines inside a cell stay in that cell's text.
5. **INSIDE vs OUTSIDE GRID**: Do not absorb outside-grid headings/paragraphs into the
   table. End the table when the grid ends.
6. **REPEATS HEADER**: Only set `repeats_header` on continuation tables (boundary is
   "{ItemBoundary.RESUMED.value}" or "{ItemBoundary.BOTH.value}"). Set true/false if clear;
   null if unsure.
...
If you emit a FIGURE block:
  - figure.alt_text MUST be present (not null) and non-empty (≤ ~500 chars). Describe what
    is visible.
  - Set figure.figure_kind to one of {allowed_figure_kinds}; keep conservative. If
    figure_kind is "equation", figure.contains_text must be true.
