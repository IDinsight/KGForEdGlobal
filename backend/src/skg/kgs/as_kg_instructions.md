...
For v0, review sets must remain small. The implemented max review set size is 12 candidates. Oversized connected components are split by conservative source-derived context:
...
candidate.description remains candidate-level semantic text from the extraction LLM.
final SFI description should be source-backed and deterministic enough to audit.
