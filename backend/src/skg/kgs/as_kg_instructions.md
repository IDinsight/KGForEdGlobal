...
Python should not hard-code country-specific heading level grammar. The generic prompt can explain how to interpret ordered heading evidence, and `RunConfig.kgs` / `CreateKGConfig` should supply document-specific hierarchy grammar such as Grade -> Strand -> Sub-Strand -> Content Standard -> Indicator when available.
...
2. Validate that final records are UUID/identity/candidate-coverage safe and that no conflict or needs_review groups were silently included.
3. Build lookup indexes over finalized SFIs by final UUID, statement type, normalized code, raw code, source windows, source segments, source context keys, and source order.
4. Recover source context for each final SFI from DocumentIR and final-record provenance.
5. Persist sfi_final_contexts.json as a debug artifact if useful.
6. Build bounded, source-grounded candidate parent sets for every final SFI.
7. Treat code-parent matches as evidence only when normalized codes are present, unique, compatible, and not contradicted by audit/source-context evidence.
8. Treat same-code/different-content audited records as separate final nodes; never collapse or select by code alone.
9. Treat raw statement_code values with null normalized_statement_code as source-visible evidence only.
10. Run LLM-assisted parent selection over batches of children, with each child receiving its own candidate parent set and the StandardsFramework root fallback.
11. Validate every response for exact child coverage, known parent IDs, no self-loops, no invented relationship types, and one direct parent decision per child.
12. Assemble deterministic hasChild edges, preserving unresolved/root-fallback status where needed.
13. Validate graph-level constraints: every finalized SFI has one parent decision, every endpoint exists, no cycles, no duplicate direct parents, and all relationship IDs are deterministic.
14. Persist final_has_child_edges.json, unresolved_edges.json, request/response JSONL, candidate parent sets, and has_child_resolution_summary.json.
```
