...
2. JSON files:
    ...
    - `document_ir.json`: the stitched DocumentIR output for Senegal's reading curriculum PDF
    - `kg_run_manifest.json`: output from the KG prep stage for the Senegal's reading curriculum
...
The next coding target is Step 6: build a lean global candidate registry from `sfi_extraction_results.jsonl` and `extraction_windows.jsonl`. Step 6 should validate that results align to the current windows, flatten window-local candidates into document-level registry records, compute lightweight text/code bucket keys for later duplicate review, and write a small summary. It should not merge candidates, mint final SFI IDs, infer hierarchy, or perform full source-context recovery. After candidate merge/dedup and final SFI ID minting, recover section paths/source context from the DocumentIR and use those signals to resolve `hasChild` edges.
...
### Step 7. Merge duplicate candidates globally — next step
...
Step 7 consumes `sfi_candidate_registry.json` from Step 6. Treat Step 6 duplicate buckets as review inputs, not decisions. Step 7 may merge candidates from outside a bucket when source provenance makes the duplicate obvious, but for v0 it should prefer conservative deterministic rules and small bounded review sets over broad semantic clustering.

Expected duplicate sources:

```text
overlapping extraction windows
row-overlap repetition in chunked tables
rows_filldown or row-span repetition
same official code appearing in multiple windows
same source table row exposed in adjacent windows
same heading/grouping/context surfaced in both block and table windows
continuation tables split across pages
source-visible bilingual pairs when config policy allows pairing
```

For coded curricula such as Ghana and Zambia:

```text
Primary merge key: statement_type + normalized_statement_code.
Merge same statement_type + same normalized_statement_code when text and source context are compatible.
Do not merge candidates with different official codes solely because the normalized text is similar.
Treat same-code candidates with materially conflicting text or incompatible context as conflict/review groups, not silent merges.
...
```text
Primary merge evidence: same statement_type + same normalized source/description text + compatible source context.
Context must include the available source-derived disambiguators: source segment, table local_code, table row/header indexes, section/level/stage/domain/palier/week context, etc. when recoverable from candidate/window references, and nearby source order.
Do not globally merge repeated labels or similar no-code objectives by text alone. Labels such as CE level, palier, week, domain, grade, component, topic, or section headings may repeat under different contexts.
```

For bilingual or mixed-language curricula:

```text
Follow config.bilingual_pair_policy.
```

Merge outputs should preserve enough information for Step 8 and Step 9:
...
For v0, it is acceptable to leave ambiguous buckets as `needs_review` or `conflict` instead of forcing a merge.
...
Start from validated Step 4/5 artifacts:
...
For v0, Step 6 should be a lean deterministic registry builder. It should support both no-code curricula such as Senegal Reading/Senegal Math and coded curricula such as Ghana/Zambia without branching into separate pipelines.
...
```text
1. Load and validate SFI extraction results.
2. Load the matching extraction windows.
3. Verify result/window alignment by window_id, window_index, and source_segment_ids.
4. Flatten every window-local SFICandidate into a registry record with a temporary registry_candidate_id.
5. Preserve the original candidate payload plus window_id, window_index, and source_segment_ids.
6. Compute normalized text keys and normalized code keys when codes exist.
7. Build possible duplicate buckets by code and by text.
8. Write sfi_candidate_registry.json with a small summary and warnings.
```

Do not implement merge, final IDs, hierarchy, full DocumentIR source-context recovery, or auxiliary policy coverage in Step 6. Candidate duplicate buckets are inputs for Step 7 only.
...
After the lean candidate registry is implemented, continue with:
