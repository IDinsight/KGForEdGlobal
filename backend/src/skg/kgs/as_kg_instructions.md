...
2. JSON files:
    - `config_literacy_and_language_curriculum.json`: the runtime config; its `kgs` section contains both KG runtime controls and country/document-specific Academic Standards extraction/config attributes
    - `document_ir.json`: the stitched DocumentIR output for Zambia's curriculum PDF
    - `kg_run_manifest.json`: output from the KG prep stage for Zambia's curriculum

---

## Current architecture

The overall pipeline has three conceptual layers. Layers A and B are already implemented. Layer C is implemented through finalized SFI records in v0; Step 9 `hasChild` relationship resolution and final Academic Standards artifact compilation are the next implementation targets.

### Layer A: PageIR extraction and verification
...
The vision/LLM extraction layer does not decide KG semantics. That is, it does not decide that something is a grade node, standard, objective, descriptor, activity, etc. Its job is to preserve page content faithfully.
...
NB: The DocumentIR is still layout/source material. It does **not** yet know which content is a standard, a grouping, an example, an activity, a descriptor, or a competency.
...
This is the KG creation layer. In the current v0 state, it is implemented through deterministic final SFI record creation with the help of LLM at critical points. The next unimplemented piece is Step 9 direct `hasChild` relationship resolution, followed by final Academic Standards KG compilation/export and validation.

The KG creation layer takes a stitched `DocumentIR` plus country/document-specific Academic Standards attributes from `RunConfig.kgs` / `CreateKGConfig` and uses LLM-driven semantic extraction, followed by deterministic validation/merge/finalization/export, to turn the source material into validated KG-shaped artifacts.

For v0, layer C should focus on Academic Standards only:
...
In SFI extraction results, `candidate.source_text` is a source-visible evidence quote for validation and review. It is not the final canonical KG statement text and must not be treated as the only downstream provenance. `candidate.description` is the candidate-level semantic statement text, but it must still be source-supported: validators should reject descriptions that cannot be found directly in, or conservatively assembled from, source-visible text in the persisted `ExtractionWindow`.

For table-derived candidates, `description` must be supported by the candidate's cited `table_row_indexes` and/or `table_header_indexes`, not merely by text visible somewhere else in the same table window. If the description uses visible continuation fragments across adjacent source rows or cells, the extraction result must include every contributing `table_row_index` and/or `table_header_index`. The model must not clean, translate, paraphrase, infer parent context, or complete the description from uncited rows/headers. When unsure, use an exact source-visible excerpt as both `description` and `source_text`.

Later registry, merge, and export stages should recover full source context from the persisted `ExtractionWindow` and stitched DocumentIR rather than relying on `source_text` alone.
...
The first complete product should be an Academic Standards KG artifact bundle. Do not implement LearningComponents or LearningProgressions until the SFI hierarchy is working on at least Ghana, Zambia, and Senegal Reading curricula. When those phases are added, keep their relationship semantics separate: LearningComponents support SFIs; LearningProgressions connect SFIs to other SFIs.
...
### 3. Keep Learning Commons ontology boundaries explicit

A `StandardsFrameworkItem` may be either:

```text
source-visible organizational/structural element within a standards framework
  examples: grade band, domain, strand, cluster, topic, sub-topic, palier, week grouping, etc.

source-visible normative learning expectation
  examples: standard, content standard, indicator, objective, specific competence, etc.
```
...
### 4. Config-driven, not country-hardcoded
...
This field should tell the dedup LLM when candidate records should merge, stay separate, be marked as conflicts, or remain unresolved for review. Keep `bilingual_pair_policy` separate because it is narrower, optional, and especially relevant for bilingual source-local pairing.
...
### 6. Academic Standards extraction is LLM-driven by design
...
### 7. No-code support is mandatory
...
### 8. Preserve original language and provenance
...
### 9. Treat table helper views as helpful but optional
...
### 10. Use candidates before final KG nodes
...
### 12. Relationship endpoint rules
...
for table-derived candidates, description must be supported by the cited `table_row_indexes` and/or `table_header_indexes`; whole-window table text is not sufficient support
language
...
When `overwrite=False`, Step 7 may reuse a complete current merge report only after validating that the persisted report can be rebuilt exactly, or content-validated equivalently, from the current registry candidate payloads, current planned review requests, saved validated review responses, current unresolved/singleton grouping logic, and companion JSON/JSONL artifacts. Validation must compare the rebuilt/current merge groups against the saved merge groups and must still verify exact registry-candidate coverage. Matching `registry_candidate_id` values alone is insufficient because registry IDs can remain stable while candidate descriptions, source text, source references, confidence, audit annotations, or merge-group evidence have changed. If a complete report is unavailable or not content-current, Step 7 may resume from a valid review request/response prefix and rewrite JSONL progress to a clean completed prefix before continuing.
...
### Step 8. Mint deterministic final SFI IDs — implemented
...
### Step 9. Resolve final hasChild edges after SFI finalization - next step
...
For v0 Academic Standards KG export, a fully reachable graph is preferable, but unresolved root fallbacks must remain visible in debug/validation artifacts so they are not mistaken for high-confidence semantic hierarchy.
...
```python
validate_academic_standards_kg(artifacts, kg_dirs) -> Path
```
...
Zero-window or zero-SFI output fails.
```
...
Lessons from reviewed Senegal Step 8 artifacts:
