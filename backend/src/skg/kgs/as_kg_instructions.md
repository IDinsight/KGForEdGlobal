...
The next coding target is Step 6: build the global candidate registry from `sfi_extraction_results.jsonl`. After candidate merge/dedup and final SFI ID minting, recover section paths/source context from the DocumentIR and use those signals to resolve `hasChild` edges.
...
language
source segment/table/row/header/cell provenance
source window IDs and source segment IDs needed for post-dedup context recovery
```

The registry must treat candidate `source_text` as evidence text only. It may be useful for hashing, review, and source-grounding validation, but it is not sufficient by itself for final SFI provenance. Registry records should retain enough source references to recover the complete source-backed statement and surrounding context from `extraction_windows.jsonl` and/or `document_ir.json`, especially when a table statement is split across multiple rows/cells or when a grouping candidate is derived from table headers.
...
Start from `sfi_extraction_results.jsonl` and `sfi_extraction_summary.json` after the Ghana Step 4 smoke/full run. The next session should inspect output quality before implementing merge/dedup:

```text
Are Basic/grade, strand, and sub-strand headings extracted as Standard Grouping candidates?
Are content standards and indicators extracted as Standard candidates?
Are examples, exemplars, core competencies, activities, resources, and teacher guidance excluded from SFI candidates or placed only in auxiliary_candidates?
Are table_row_indexes populated correctly for row-derived table candidates?
Are table_header_indexes populated correctly for header-derived table candidates?
Are source_text values source-visible evidence quotes and not paraphrased?
Are final SFI descriptions/provenance recovered from source references rather than copied blindly from truncated source_text?
Are parent_references and ancestor_context_references source-visible hints only?
Are code-parent hints treated as hints rather than final hasChild edges?
Are empty/front-matter windows represented as valid zero-candidate extraction results?
```
...
After the candidate registry is implemented, continue with:
