...
The current v0 implementation now covers the prep, base extraction-window artifact, and Step 4/5 SFI extraction artifact stages:
...
9. Validate each SFIExtractionResult with schema validation and source-grounding quality checks.
...
Step 6 now builds a lean global candidate registry from `sfi_extraction_results.jsonl` and `extraction_windows.jsonl`. It validates result/window alignment, flattens window-local candidates into document-level registry records, computes lightweight text/code bucket keys for later duplicate review, and writes a small summary. It does not merge candidates, mint final SFI IDs, infer hierarchy, or perform full source-context recovery. The next coding target is Step 7: use bounded LLM-assisted deduplication over registry review sets to decide which candidates represent the same final source item. After candidate merge/dedup and final SFI ID minting, recover section paths/source context from the DocumentIR and use those signals to resolve `hasChild` edges.
...
Use the LLM as the primary semantic extractor for Academic Standards v0. The LLM returns one `SFIExtractionResult` per extraction window, and that result must validate against the extraction response schema plus source-grounding quality checks. Deterministic code should not bypass this LLM candidate-extraction stage except for mechanical framework-root creation and later validation/export bookkeeping.
...
source-grounding checks for codes, source_text, references, table_row_indexes, and table_header_indexes
...
Compute only lightweight keys, such as:

```text
normalized_description
normalized_source_text
normalized_statement_code, when statement_code exists
text_bucket_key = statement_type + normalized_description
source_text_bucket_key = statement_type + normalized_source_text
code_bucket_key = statement_type + normalized_statement_code, when statement_code exists
```

For coded curricula such as Ghana and Zambia, create possible duplicate buckets by:

```text
statement_type + normalized_statement_code
statement_type + normalized_description
```

For no-code curricula such as Senegal Reading and Senegal Math, create possible duplicate buckets by:

```text
statement_type + normalized_description
statement_type + normalized_source_text
```
...
### Step 7. Merge duplicate candidates globally with bounded LLM dedup review — next step
...
merge_sfi_candidates(
    *,
    candidate_registry: SFICandidateRegistry,
    config: CreateKGConfig,
    dedup_agent: SFIDedupAgent,
) -> SFIMergeResult
```
...
```text
Collapse repeated registry candidates into one logical SFI merge group per final source item. Step 7 decides which candidates represent the same curriculum item and merges their source references. It does not mint final SFI IDs, create hasChild edges, recover full DocumentIR source context, or compile final KG objects.
```

Step 7 consumes `sfi_candidate_registry.json` from Step 6. Treat Step 6 duplicate buckets and warnings as review-set construction inputs, not merge decisions.

The Step 7 design is LLM-assisted by default, but bounded by deterministic Python retrieval and validation:

```text
Python builds small source-grounded dedup review sets.
The dedup LLM decides merge / keep separate / conflict / needs_review within each supplied set.
...
Do not send the whole registry to the dedup LLM. Do not iterate candidate-by-candidate. Iterate over bounded review sets built from duplicate buckets, warnings, and obvious provenance overlap.
...
config.sfi_deduplication_instructions
config.bilingual_pair_policy, when present
```

`config.sfi_deduplication_instructions` is the single v0 field for curriculum-specific duplicate handling. It should cover the old conceptual scope of duplicate-review guidance and repeated-statement policy in one concise instruction. Do not maintain separate duplicate-review and repeated-statement instruction fields for Step 7.

Keep `bilingual_pair_policy` separate because bilingual pairing is optional, narrower, and may apply only to source-local Wolof/French or mixed-language pair decisions.

The dedup prompt payload should not include broad registry/document metadata such as country, subject, framework title, or registry summary. Those fields are useful for logs and reports, but they do not materially improve dedup decisions. The prompt should include only:
...
Create initial review sets from these sources when available:

```text
same code duplicate buckets
same description-text duplicate buckets
same source-text duplicate buckets
same-code/different-description warning groups
same-text repeated within-window warning groups
same-text repeated across-window warning groups
overlapping extraction-window/table-row provenance
same source table row or adjacent overlapped table chunks
source-local bilingual-pair candidates when config.bilingual_pair_policy exists
```

Then merge overlapping review sets into connected components before calling the LLM. For example:
...
=> one dedup review set: c1, c2, c3, c4
```

This avoids contradictory decisions across multiple LLM calls.

For v0, keep review sets small. If a connected component is very large because of repeated generic labels, split it by safe source-derived context such as statement_type, statement_code, source segment, table segment, grade/level/component context, or source-order neighborhood. If it still cannot be bounded safely, mark it `needs_review` rather than asking the LLM to solve a broad clustering problem.

#### Dedup review request payload

Each LLM review request should include a compact candidate view, not the full registry or full DocumentIR. Include fields needed for dedup only:
...
Python must reject, retry, or mark the set unresolved if the LLM response:
...
returns an empty reason for a non-singleton decision
violates hard constraints configured for the curriculum
```
...
Context can include source segment, table local_code, table row/header indexes, extraction window overlap, section/level/stage/domain/palier/week context when already available in candidate/window references, and nearby source order.
...
```text
Follow config.bilingual_pair_policy when present.
```
...
Step 7 should produce merge groups for every registry candidate. Candidates not included in any LLM review set become singleton groups automatically.

Each merge group should preserve enough information for Step 8 and Step 9:

```text
merge_group_id, temporary and deterministic for the report
merged registry_candidate_ids
statement_type and normalized_statement_type
statement_code / normalized_statement_code when present
candidate descriptions and source_text evidence retained for audit
merged source window IDs, source segment IDs, table row indexes, table header indexes, and confidence range
merge_decision: merged | singleton | conflict | needs_review
merge_reason: short deterministic explanation or LLM review reference
```

For v0, it is acceptable to leave ambiguous groups as `needs_review` or `conflict` instead of forcing a merge.
...
```python
mint_final_sfi_ids(config: CreateKGConfig, document_ir, merge_result) -> list[FinalSFIRecord]
```
...
Start from the validated Step 6 artifact:

```text
sfi_candidate_registry.json
```

For v0, Step 7 should be a bounded LLM-assisted dedup stage. It should support both no-code curricula such as Senegal Reading/Senegal Math and coded curricula such as Ghana/Zambia without branching into separate pipelines.
...
```text
1. Load and validate sfi_candidate_registry.json.
2. Index candidates by registry_candidate_id.
3. Build bounded dedup review sets from duplicate_buckets, warnings, and obvious provenance overlap.
4. Merge overlapping review sets into connected components.
5. Send each bounded review set to the dedup LLM with the general dedup system instruction, config.sfi_deduplication_instructions, optional config.bilingual_pair_policy, review reasons, and compact candidate records.
6. Validate each LLM response against the closed decision enum and candidate-ID coverage rules.
7. Convert accepted LLM decisions into merge groups.
8. Create singleton merge groups for candidates not included in any review set.
9. Write dedup review requests/responses and merge report artifacts.
```

Do not implement final IDs, hierarchy, full DocumentIR source-context recovery, final canonical text selection, or auxiliary policy coverage in Step 7. Dedup LLM decisions are inputs for Step 8 final SFI record creation.

Lessons from reviewed Step 4/5 artifacts:
...
Ghana: stable alphanumeric statement codes are strong duplicate-bucket signals, but same-code conflicts should be warnings, not Step 6 merges.
...
Before implementing Step 7 merge/dedup, inspect the Step 6 registry summary:

```text
Do candidate counts match sfi_extraction_summary.json?
Are coded candidates bucketed by statement_type + statement_code?
Are no-code candidates bucketed by statement_type + normalized text?
Are same-code/different-description cases reported as warnings?
Are repeated same-text buckets clearly marked as possible duplicates, not merged records?
Are within-window duplicate candidates visible in duplicate buckets?
Are table_row_indexes and table_header_indexes preserved exactly from extraction results?
```

After Step 7 merge/dedup is implemented, continue with:
