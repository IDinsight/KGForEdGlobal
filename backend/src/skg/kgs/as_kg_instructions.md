...
12. Resume or skip Step 4/5 from existing JSONL and summary artifacts when overwrite is false.
```

Do **not** add a separate context-enrichment pass to `extraction_windows.jsonl` for v0. The persisted extraction windows should stay source-faithful and include source text, block/table payloads, provenance, table helper views, profile instructions, code matches, and code-parent hints. Broader source context such as `section_path` should be recovered later from the DocumentIR after SFIs have been finalized and should be used during `hasChild` relationship resolution.

The persisted `ExtractionWindow` remains the complete source-faithful artifact and the validation source of truth. Step 4 may derive a compact prompt payload from each `ExtractionWindow` to reduce token cost and model distraction. The compact prompt should preserve only the fields needed for SFI extraction: window identity, segment kind, block/table source text, compact table rows, source row indexes, compact provenance, code matches, code-parent hints, and profile extraction instructions. Validators should still compare LLM outputs against the full `ExtractionWindow` artifact.
...
descriptor/guidance/activity text   -> auxiliary candidate, metadata, dropped text, or Other only if profile policy says so
...
```text
Use the LLM to convert each persisted source-faithful extraction window into candidate StandardsFrameworkItems and auxiliary records, guided by the DocumentProfile and a compact prompt-facing source payload derived from the full ExtractionWindow.
```
...
compact block payload or compact table row payload
table row indexes for table windows
compact source provenance
code matches and code-parent hints
DocumentProfile extraction instructions
```
...
source row indexes for table-derived candidates
...
source-grounding checks for codes, source_text, references, and table_row_indexes
...
source_text copied or faithfully excerpted from visible source text
...
table_row_indexes for table-derived candidates
...
when overwrite is false and both result and summary artifacts already exist, skip Step 4/5 and return the existing results
...
Are table_row_indexes populated correctly for table-derived candidates?
Are source_text values source-visible and not paraphrased?
...
The SFI extraction prompt should focus on extracting SFI and auxiliary candidates. The persisted full `ExtractionWindow` remains the source-faithful validation artifact, while Step 4 may send a compact prompt payload to reduce token cost and model distraction. The hasChild-resolution prompt should run after final SFI records exist; it should receive each child's recovered raw `section_path`, source provenance, and candidate parent SFIs from the finalized registry, then choose the direct parent or mark unresolved.
