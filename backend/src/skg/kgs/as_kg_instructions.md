...
### Step 6. Build a lean global candidate registry
...
normalized_description_without_leading_code
normalized_source_text_without_leading_code
normalized_statement_code, when statement_code exists
text_bucket_key = statement_type + normalized_description_without_leading_code
source_text_bucket_key = statement_type + normalized_source_text_without_leading_code
code_bucket_key = statement_type + normalized_statement_code, when statement_code exists
```

The `*_without_leading_code` fields are important for coded curricula such as Zambia, where one candidate description may include a visible code prefix and another duplicate candidate may not. Strip only obvious leading source-code prefixes; do not remove meaningful mathematical, grade, or objective text in the middle of a statement.
...
statement_type + normalized_description_without_leading_code
```
...
```text
statement_type + normalized_description_without_leading_code
statement_type + normalized_source_text_without_leading_code
```
...
```python
merge_candidates(candidate_registry, config: CreateKGConfig) -> SFIMergeResult
```

Purpose:

```text
Collapse repeated candidates into one final logical SFI record with merged provenance and enough source references to recover DocumentIR context for relationship resolution. Do not choose final SFI text or provenance solely from a single candidate's `source_text`; recover and reconcile source-backed text from merged window, segment, row, header, and DocumentIR references.
```
...
Zambia: hierarchical numeric statement codes are useful, but descriptions may or may not include the leading code; compute text keys with obvious leading code prefixes stripped.
```
...
Are leading source-code prefixes stripped from text keys where appropriate?
