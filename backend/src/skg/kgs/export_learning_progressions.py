...
def _emit_relationship(
    *,
    ...
    ------
    ValueError
        If the candidate's inference type is unsupported or if the SFI context index is
    ...
    if candidate.inference_type in WITHIN_LEVEL_INFERENCE_TYPES:
        ...
        raise ValueError(
            f"Unsupported LP inference_type during relationship emission: "
            f"{candidate.inference_type!r}"
        )
    ...
    if sfi_context_index is not None:
        ...
        if source_context is None or target_context is None:
            raise ValueError(
                f"Missing SFI context during LP relationship emission: "
                f"source={candidate.source_sfi_uuid} found={source_context is not None}; "
    ...
    return Relationship(
        ...
        date_created=None,
        date_modified=None,
        description=f"{candidate.rel_type} relationship inferred between StandardsFrameworkItems.",
        identifier=uuid5(
