...
8. Mint deterministic final SFI records from merge groups eligible for automatic finalization and report any finalization exclusions in the summary.
...
- Finalization exclusion counts are reported from `sfi_final_summary.json`; Step 10 uses summary counts only for finalization exclusions.
- Unresolved/root-fallback relationship cases are reported from `has_child_unresolved_edges.json`.
- Zero-final-SFI or empty-final-KG output fails.
...
finalization_exclusion_summary:
  - exclusion count fields copied from sfi_final_summary.json
```
...
3. Load or receive the required Step 10 inputs; use summary counts only for finalization exclusions in Step 10 compile/export.
...
13. Wire the final function into `create_kgs.py` as the last pipeline step and add focused tests for happy path, no-code curricula, same-code/different-content, root fallback, dangling edges, missing incoming edges, cycles, stale artifacts, finalization exclusion count reporting, zero-final-SFI failure, empty-final-KG failure, and serialization round trip.
...
- Split oversized components conservatively; unresolved residues are carried forward as non-finalized review groups.
- LLM decisions determine which candidate groups merge, stay separate, or require exclusion from automatic finalization.
...
- Exclude non-finalizable merge groups from automatic final SFI records and report exclusion counts.
...
- Fail before relationship resolution if finalization excluded any merge groups, unless an explicitly supported incomplete-universe policy exists.
...
- Report unresolved relationship cases from `has_child_unresolved_edges.json` and finalization exclusion counts from `sfi_final_summary.json`; use summary counts only for finalization exclusions.
...
Zero final SFIs or empty final KG artifacts should fail by default. Zero extraction windows are handled earlier and are not a Step 10 concern.
