...
- statement_code must be an exact source code supported by the configured code policy. Use the exact block.local_code or table.local_code when that metadata is the item's visible code and it matches the candidate statement_type's configured code_type. Otherwise, use only an exact code_matches[].value whose code_type matches statement_type_policy.code_type for the candidate. When statement_type_policy.code_type is null, emit a code only when it unambiguously matches exactly one configured code pattern. Use null for nearby codes, table identifiers that are not item codes, ambiguous codes, or codes that belong to another statement.
...
- When statement_code comes from code_matches rather than block.local_code/table.local_code, source_text must contain that exact standalone code directly paired with the complete description. Use only "code + complete description", "complete description + code", or the complete description itself when the code is already part of it. Do not include another statement, another code, or unrelated surrounding text merely to make a code visible.
...
- Treat block.local_code and table.local_code as source-visible metadata evidence only when the exact value is the candidate item's code and matches the applicable configured code pattern; do not automatically copy a table identifier or segment label into statement_code.
