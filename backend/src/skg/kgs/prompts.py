...
- If any supplied non-root candidate has `source_visible_direct_parent` evidence, choose from those source-visible direct parent candidates instead of choosing the StandardsFramework root or a weaker non-root candidate that is supported only by semantic similarity, page/window proximity, same segment, same source window, same topic wording, or inferred code hierarchy.
