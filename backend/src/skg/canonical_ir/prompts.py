...
- Treat headings containing “tolluwaay” and/or “étape” as stage identity markers, but do NOT automatically assign level 1. Assign level 1 only when the heading clearly starts a major section/subtree (i.e., it is a top-level stage container for many subsequent pages/sections). If it appears as an in-section marker inside competence/front-matter (e.g., between “Compétence de cycle” and “Compétence de l’étape” or under “Cadre de compétences”), assign a deeper level consistent with the surrounding competence structure (often level 3–5), using adjacent headings in the ordered list as evidence.
...
- For weekly "Planification" tables (typically Tableau 2–3): the document is GRADE-first. Treat the table as a grade-scoped UNIT (Planification CE1/CE2). Even if the caption contains multiple strands (e.g., "Activités numériques et Résolution de problèmes"), DO NOT set STRAND in context_groupings. Instead, represent strands row/column-locally using the column-header fanout pattern (STRICT — REQUIRED):
  - IMPORTANT (validator constraint): When using row/column-local STRAND groupings for Planification tables, do not include GRADE_LEVEL in context_groupings. Even if the section_path includes 'CE1'/'CE2', put grade scoping in SegmentDecision.groupings[] instead (and optionally emit a UNIT grouping under it). Keep context_groupings limited to outer document context such as SUBJECT/STAGE/SECTION for Planification.
