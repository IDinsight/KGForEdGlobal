...
def _table_cell(text: str | None, col_span: int = 1, row_span: int = 1) -> TableCell:
...
def _table_row_with_spans(cells_data: list[tuple[str | None, int]]) -> TableRow:
...
def test__finalize_table_structure_computed_cols_via_spans(
    ...
    complex_row = _table_row_with_spans([("Col 1", 1), ("Col 2-4", 3)])
    ...
    n_cols, _, _ = utils._finalize_table_structure(
        chain=chain,
        header_rows=[],
        local_code=None,
...
def test__finalize_table_structure_empty_headers(
    ...
    stitched_rows = [_table_row("1", "2")]
    chain = [(0, 0, create_table(2))]
    warnings: list[str] = []

    _, sig, headers_canon = utils._finalize_table_structure(
        chain=chain,
        header_rows=[],  # Empty
        local_code=None,
        segment_id="seg_2",
...
def test__finalize_table_structure_inflation_warning(
    ...
    # Computed = 4.
    stitched_rows = [_table_row("A", "B", "C", "D")]
...
def test__finalize_table_structure_max_declared_in_chain(
    ...
    stitched_rows = [_table_row("A")]  # Computed = 1
...
def test__finalize_table_structure_normalization_and_signature(
    ...
    #   Row 2: "Sub 1", "Sub 2"
    header_rows = [_table_row("  Header A ", "HEADER B"), _table_row("Sub 1", "Sub 2")]

    # Stitched rows usually contain headers + body
    stitched_rows = header_rows + [_table_row("val1", "val2")]

    chain = [(0, 0, create_table(2))]
    warnings: list[str] = []

    n_cols, sig, headers_canon = utils._finalize_table_structure(
        chain=chain,
...
def test__populate_grid_spans_col_span(
    ...
    utils._populate_grid_spans(segment=create_segment, grid=grid, n_rows=1, n_cols=3)
...
def test__populate_grid_spans_explicit_padding(
    ...
    utils._populate_grid_spans(segment=create_segment, grid=grid, n_rows=2, n_cols=2)
...
def test__populate_grid_spans_trailing_padding_ignored(
    ...
    # Should not raise.
    utils._populate_grid_spans(segment=create_segment, grid=grid, n_rows=1, n_cols=2)
...
def test__populate_grid_spans_header_count_mismatch_warning() -> None:
    ...
    next_item = Table(
        ...
        kind="table",
        local_code=None,
        rows=[_table_row("R1"), _table_row("R2"), _table_row("R3")],
        repeats_header=None,
        header_row_count=3,  # Mismatch > segment
    )
    warnings: list[str] = []

    utils._process_next_table_slice(
        current_local_code=None,
        next_item=next_item,
        next_item_index=1,
        next_page_index=1,
        segment_header_row_count=segment_hrc,
        segment_header_rows=[_table_row("H1"), _table_row("H2")],
        segment_id="seg_1",
        warnings=warnings,
    )
...
def test__process_next_table_slice_explicit_header_drop() -> None:
    ...
    header_rows = [_table_row("H1"), _table_row("H2")]

    # 2 Header rows, 1 Data row. repeats_header=True.
    next_rows = [_table_row("H1"), _table_row("H2"), _table_row("Data")]
    next_item = Table(
        bbox=[0, 0, 100, 100],
        boundary=ItemBoundary.RESUMED,
        header_row_count=2,
...
def test__process_next_table_slice_implicit_drop_calls_helper() -> None:
    ...
    mock_data_row = _table_row("Data Only")
    header_rows = [_table_row("H1")]
    next_rows = [_table_row("H1"), _table_row("Data Only")]
...
def test__resolve_header_row_count_inference_ignored_low_confidence() -> None:
    ...
    table = Table(
        ...
        local_code=None,
        repeats_header=None,
        rows=[_table_row("Year 2023"), _table_row("1000")],  # Score ~1.25
    )
    warnings: list[str] = []

    result = utils._resolve_header_row_count(
...
def test__resolve_header_row_count_inference_returns_zero_count() -> None:
    ...
    table = Table(
        ...
        local_code=None,
        repeats_header=None,
        rows=[_table_row("12345"), _table_row("67890")],
    )
...
def test__resolve_header_row_count_inference_success_high_confidence() -> None:
    ...
    table = Table(
        ...
        local_code=None,
        repeats_header=None,
        rows=[_table_row("Description", "Category"), _table_row("123", "456")],
    )
    warnings: list[str] = []
...
def test__resolve_header_row_count_returns_explicit_count_immediately() -> None:
    ...
    table = Table(
        ...
        header_row_count=2,
        kind="table",
        rows=[_table_row("H1"), _table_row("H2"), _table_row("Data")],
    )
...
def test__resolve_initial_local_code_all_items_empty() -> None:
    ...
    chain = [
        _make_chain_entry(0, 0, None),
        _make_chain_entry(1, 0, ""),
        _make_chain_entry(2, 0, "   "),
    ]
...
def test__resolve_initial_local_code_first_item_empty_finds_second() -> None:
    ...
    chain = [
        _make_chain_entry(0, 0, None),  # Missing
        _make_chain_entry(0, 1, "   "),  # Empty/Whitespace -> treated as None
        _make_chain_entry(1, 0, "Table 3"),  # Target
    ]
...
def test__resolve_initial_local_code_first_item_has_code() -> None:
    ...
    chain = [
        _make_chain_entry(0, 0, "Table 1"),
        _make_chain_entry(0, 1, "Table 2"),  # Should be ignored
    ]
...
def test__resolve_initial_local_code_normalization_behavior() -> None:
    ...
    # "  Table   1.2  " -> "table 1.2".
    chain = [_make_chain_entry(0, 0, "  Table   1.2  ")]
...
def test__resolve_initial_local_code_single_item_chain_none() -> None:
    ...
    chain = [_make_chain_entry(0, 0, None)]
    assert utils._resolve_initial_local_code(chain) is None
...
def test__resolve_initial_local_code_stops_at_first_valid_match() -> None:
    ...
    chain = [
        _make_chain_entry(0, 0, None),
        _make_chain_entry(1, 0, "Table A"),  # First valid
        _make_chain_entry(2, 0, "Table B"),
    ]
...
def test_expand_table_rows_to_rows_grid_bounds_validation() -> None:
    ...
    rows = [TableRow(cells=[_table_cell("A", 3, 1)])]
    segment = _make_segment(rows, n_cols=2)

    with pytest.raises(ValueError, match="col_span out of bounds"):
        utils.expand_table_rows_to_rows_grid(segment=segment)


def test_expand_table_rows_to_rows_grid_complex_span_expansion() -> None:
    ...
    rows = [
        TableRow(cells=[_table_cell("A", 1, 2), _table_cell("B")]),
        TableRow(cells=[_table_cell("C")]),
    ]
    segment = _make_segment(rows, n_cols=2)

    out_rows, sources = utils.expand_table_rows_to_rows_grid(segment=segment)
...
def test_expand_table_rows_to_rows_grid_overflow_error() -> None:
    """Verify ValueError when row content exceeds declared n_cols."""

    rows = [TableRow(cells=[_table_cell("A"), _table_cell("B"), _table_cell("C")])]
...
def test_expand_table_rows_to_rows_grid_padding_skips_occupied_slots() -> None:
    ...
    rows = [
        TableRow(cells=[_table_cell("A", 1, 2)]),
        TableRow(
            cells=[_table_cell(None), _table_cell("B")]
        ),  # Padding (empty text, 1x1)
    ]
    segment = _make_segment(rows, n_cols=2)

    out_rows, _ = utils.expand_table_rows_to_rows_grid(segment=segment)
...
def test_expand_table_rows_to_rows_grid_span_collision_error() -> None:
    """
    ...
    rows = [
        TableRow(cells=[_table_cell("A"), _table_cell("B", 1, 2), _table_cell("C")]),
        TableRow(cells=[_table_cell("D", 3)]),
    ]
    segment = _make_segment(rows, n_cols=3)
...
def test_row_provenance_by_stitched_index_length_mismatch_raises_error() -> None:
    ...
    # Slice has 1 row.
    slice_1 = _make_slice([_table_row("r1")], page_index=1)

    # Segment claims to have 2 rows.
    stitched_rows = [_table_row("r1"), _table_row("ghost_row")]

    segment = _make_segment(rows=stitched_rows, n_cols=1, seg_id="bad_seg")
    segment.slices = [slice_1]

    msg = "Row <-> slice mapping length mismatch for TableSegment bad_seg"

    with pytest.raises(ValueError, match=msg):
        utils.row_provenance_by_stitched_index(segment=segment)


def test_row_provenance_by_stitched_index_multi_slice_stitching() -> None:
    """Test provenance across two stitched slices without dropped headers."""

    slice_1_rows = [_table_row("p1_r1"), _table_row("p1_r2")]
    slice_2_rows = [_table_row("p2_r1"), _table_row("p2_r2")]

    all_rows = slice_1_rows + slice_2_rows

    slice_1 = _make_slice(rows=slice_1_rows, page_index=1, bbox=[0, 0, 50, 50])
    slice_2 = _make_slice(rows=slice_2_rows, page_index=2, bbox=[0, 0, 60, 60])

    segment = _make_segment(rows=all_rows, n_cols=1)
    segment.slices = [slice_1, slice_2]

    provenance = utils.row_provenance_by_stitched_index(segment=segment)
...
def test_row_provenance_by_stitched_index_no_slices_assertion() -> None:
    ...
    segment = _make_segment(rows=[], n_cols=1)
...
def test_row_provenance_by_stitched_index_single_slice_simple() -> None:
    ...
    slice_1 = _make_slice(rows=rows, page_index=1, bbox=[0, 0, 100, 300])

    segment = _make_segment(rows=rows, n_cols=1)
    segment.slices = [slice_1]
...
def test_row_provenance_by_stitched_index_with_dropped_headers() -> None:
    ...
    segment = _make_segment(rows=stitched_rows, n_cols=1)
    segment.slices = [slice_1, slice_2]
...
def test_stitch_table_chain_columns_signature_generation() -> None:
    ...
    p1, i1, table = _make_chain_entry(page=1, idx=0, code="T1")
    table.header_row_count = 2
    table.rows = [
        _table_row("H1", "H2"),
        _table_row("Sub1", "Sub2"),
        _table_row("D1", "D2"),
    ]

    chain = [(p1, i1, table)]

    segment = utils.stitch_table_chain(
        chain=chain,
        doc_key="doc",
        section_path=[],
...
def test_stitch_table_chain_deterministic_id() -> None:
    """Test that the segment ID is generated deterministically based on the doc_key
    and the first item's pointer.
    """

    page, idx, table = _make_chain_entry(page=1, idx=0, code="T1")
    table.rows = [_table_row("content")]
    chain = [(page, idx, table)]
    doc_key = "abc123hash"

    segment_1 = utils.stitch_table_chain(
        chain=chain,
        doc_key=doc_key,
        section_path=[],
        table_filldown_enabled=False,
    ...
    segment_2 = utils.stitch_table_chain(
        chain=chain,
        doc_key=doc_key,
        section_path=[],
        table_filldown_enabled=False,
...
def test_stitch_table_chain_explicit_header_drop() -> None:
    ...
    # Page 1.
    p1, i1, t1 = _make_chain_entry(page=1, idx=0, code="T1")
    t1.rows = [_table_row("Header"), _table_row("Row 1")]
    t1.header_row_count = 1
    t1.n_cols = 1

    # Page 2 (repeats header explicitly).
    p2, i2, t2 = _make_chain_entry(page=2, idx=0, code="T1")
    t2.rows = [_table_row("Header"), _table_row("Row 2")]
...
def test_stitch_table_chain_filldown_disabled() -> None:
    """Verify rows_filldown is None when disabled."""

    p1, i1, table = _make_chain_entry(page=1, idx=0, code="T1")
    table.rows = [_table_row("A", "1"), _table_row("", "2")]
...
def test_stitch_table_chain_filldown_logic() -> None:
    ...
    table.rows = [
        _table_row("Group", "Value"),
        _table_row("A", "1"),
        _table_row("", "2"),  # Empty string
        _table_row("B", "3"),
        _table_row(None, "4"),  # None value
    ]
...
def test_stitch_table_chain_propagates_local_code() -> None:
    """Test that local_code is resolved from the first non-null occurrence
    and backfilled/propagated.
    """

    # Item 1: No code.
    p1, i1, t1 = _make_chain_entry(page=1, idx=0, code=None)
    t1.rows = [_table_row("r1")]

    # Item 2: Has code "Table 2.1".
    p2, i2, t2 = _make_chain_entry(page=2, idx=0, code="Table 2.1")
    t2.rows = [_table_row("r2")]

    # Item 3: No code (should inherit "Table 2.1").
    p3, i3, t3 = _make_chain_entry(page=3, idx=0, code=None)
    t3.rows = [_table_row("r3")]
