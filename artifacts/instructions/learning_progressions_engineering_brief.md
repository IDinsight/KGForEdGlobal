...
The prospective Step 26 checkpoint-format restriction in D13-C3 applies to new production execution/reuse, not compatible historical snapshot validation. Preserve the approved independent early work above. Validate old evidence with its compatible historical code/schema without upgrading receipts or relabeling it as concurrency evidence. An absent producing SHA remains an unresolved input-binding limitation under the existing requirements, not permission to invent it or a new restriction introduced by concurrency; see Section 3.3.3.
...
**Historical snapshot boundary.** New-production format rejection does not invalidate preserved historical results or require their regeneration merely because concurrency or this format restriction was introduced. Completed graph files remain available for downstream reading, querying, analysis and consumption. Historical validation/use that interprets their schemas must employ compatible historical code/schema and retain original raw config and recorded material/artifact identities. This is separate from invoking the new production pipeline against an old directory to resume, reuse checkpoints or rewrite outputs, which remains blocked even for a completed run. Do not edit historical evidence to make current schemas accept it. D12.1.1 early evaluator work and its fixed review base remain approved independently; the format gate creates no new prerequisite for that work. Existing D12 completeness, frozen-input and exact producing-code binding requirements remain: a producing SHA absent from records is unknown, not the inspection/reconstruction SHA, review base or current HEAD. The Madhi observations in Section 3.3.3 certify neither Step 27 nor Step 28 completion and cannot be relabeled as concurrency execution evidence.
...
After new Step 26 reviewer approval, record the effective production concurrency value, exact reviewed candidate, supported checkpoint format and actual material inputs for newly authorized Step 27 execution. Check the selected shutdown/reuse/accounting contract wherever exercised. Previously started runs retain their original bindings under Sections 3.3.2–3.3.3 and are not silently converted to new-candidate evidence. The reviewer assesses retained historical snapshots using compatible historical code/schema and actual material evidence. Concurrency or format removal alone requires no Madhi regeneration; no automatic retained-snapshot approval, missing-SHA waiver or fresh-run authorization follows.
