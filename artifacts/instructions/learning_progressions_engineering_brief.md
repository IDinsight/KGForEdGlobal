...
**Status update — 2026-09-18, historical compatibility removal selected; implementation approval pending:** The user requested removal of historical readers because they will regenerate legacy artifacts. Sections 1.6, D12.1.5 and 3.3.9 specify current-format-only evaluator inputs, rejection before freezing/calls, removal of legacy configuration/request shims and unchanged integrity obligations. This governance-only phase precedes explicit approval of the amended brief and a separate coding phase in this task. The existing K=21/F=25 range and fixed review base remain. Earlier dated compatibility guarantees below are historical and are superseded for this scope only upon approval of Section 3.3.9. This phase changes no production/test code, results, prior evidence or active runs.
...
**SETTLED — Current-format-only evaluator inputs; implementation approval pending under Section 3.3.9.**

The user no longer requires historical compatibility and will regenerate legacy artifacts separately. The evaluator accepts only the current Learning Commons-shaped AS+LC+LP delivery projections defined above. Remove the internal snake_case projection reader and reject old, mixed, malformed or inconsistent projections without fallback, normalization or migration. Reconcile exact expected records against the authoritative bundle with JSON-type-sensitive comparisons, preserving original byte/material hashes and all source/config/request/judgment/provenance bindings. The F1 boolean-versus-number guard remains required for current-format validation.

Projection format and checkpoint format remain separate validation requirements. Require complete supported journal-bearing checkpoint evidence under D13-C3, including authenticated pending-completion and attempt/usage journals and their exact receipt coverage. Reject prefix-only, previously upgraded or compatibility-counter-bearing checkpoints (including zero-valued counters), missing captured capacity, and unsupported captured configuration shapes. Use exact current effective configuration and request identities; remove old-format serialization/default-suppression and identity-rebinding shims. Omission of capacity in new runtime configuration still resolves through the existing default; its resolved value must be recorded in effective execution evidence.

Apply these rules to completed-candidate validation, freezing, frozen-input loading and resume. Unsupported completed candidates fail the invocation before evaluator evidence is published or calls are dispatched; do not silently omit them as unfinished. Old frozen inputs and saved schedules are not upgraded or relabeled. Preserve source evidence unchanged on rejection, strict cache material identity, current-format reuse, locks and all integrity checks. No regeneration, migration, deletion, result rewrite or live execution is authorized here. Existing AS/AS+LC formats, internal bundle/audit schemas, optional producing-SHA provenance and Git-independent evaluation remain unchanged. D12.1.5 and Section 3.3.9 govern removal and independent testing; consumer documentation must describe current-format requirements without agent-process notes.

Step 26's internal pending-completion and usage journals under D13-C3 accompany the deterministic successful JSONL prefixes. Under the prospective 2026-09-13 amendment, new production requires complete receipt-authenticated journal-bearing checkpoint evidence; absence is not an empty journal. The transaction journal remains transient and may be absent after a committed transaction. Public graph/AS/AS+LC contracts remain intact. Historical artifacts remain unchanged as prior evidence; D12.1.5 rejects them as evaluator inputs when they do not satisfy the current format, without retrofitting journal files. Implementation approval for format removal is recorded in Section 3.3.3; separate coding/testing/reviewer gates still apply.
...
D12.1.5 removes the evaluator's historical-format exception. Early development remains permitted under the same review base and execution gates, but selected snapshots must satisfy the current projection/checkpoint/effective-configuration contract. A historical Madhi snapshot no longer substitutes for those inputs. Independent testing may construct isolated current-format synthetic fixtures; this grants no live regeneration or Step 27/28 completion. Producing SHA remains optional under D12.1.3; historical observations in Section 3.3.3 are preserved as history.
...
- **Output and resume:** write invocation-specific artifacts beneath repository-root `results/lp_evals/`, separate from production directories. Reject resolved-path aliases that could overwrite inputs. The command creates the manifest; users need not author it. Resolve resumable invocation identity deterministically from recorded root/settings and manifest identities, with unambiguous selection rather than an arbitrary latest-directory choice; concrete identifiers and optional resume controls are implementation details. Resume revalidates and uses the manifest's frozen selection and schedule, never adds newly completed discoveries, and fails if selected inputs are now missing, changed or invalid. New inputs belong to a distinct invocation; preserve earlier evidence and reuse only materially identical validated successes. Do not silently repeat completed live evaluation.
- **Unchanged gates:** both evidence-condition corrections, all D12-S/J/R/B/A settings, actual material-content binding, current-format validation under D12.1.5, independent testing/review, separate live authorization and the six reviewed project snapshots required for final completion remain mandatory. Discovery is operational selection, not reviewer approval or certification of pedagogical correctness.
...
Keep the fixed source-review base `28d4f1218c71237bc7fda1027a8ac55866bd345f`, independent deterministic testing/review, Step 27 reviewer approval, separate live authorization, all required reports/dispositions and all six reviewed snapshots for final completion. Actual evaluator implementation, configuration, input, prompt, schema, model, request, schedule, cache and report hashes remain the material identity and resume checks. Changing such material still fails compatibility validation. Development evidence remains distinct from live evaluation; no prior evidence may be relabeled.
...
### D12.1.5 SETTLED — Remove historical evaluator compatibility; implementation approval pending

The user selected current-format-only evaluation on 2026-09-18 because they will regenerate legacy artifacts separately. Section 1.6 defines the exact projection, checkpoint and captured-configuration requirements. This supersedes historical acceptance in D12.1.1–D12.1.3 and earlier compatibility guarantees, without changing optional producing provenance, Git-independent runtime operation, current production export/internal formats or independent/live/final gates.

Remove the historical internal-projection branch in evaluator `sampling.py`, prefix-only receipt acceptance, `_CapturedKGConfig` serialization used to suppress later defaults, and request/hash/identity reconstruction used only to match historical configuration shapes. Simplify dependent evaluator format discriminators, freeze/load/resume validation and public documentation as needed; do not move historical acceptance branches into another module or add a format selector. Preserve reconstruction and byte comparisons needed to independently verify current requests. Metadata recovery still needed for current-format runs, such as recovering the omitted overwrite flag by exact unique hash matching, remains an integrity obligation rather than legacy support.

A current input has the Section 1.6 Learning Commons wire projections, complete native journal-bearing checkpoint evidence validated under D13-C3, and current captured effective configuration with its resolved positive integer capacity present and equal to execution material. Historical/mixed/partial formats and missing captured capacity fail; never fill missing execution/configuration fields, normalize receipts or reseal hashes to make them acceptable. New runtime configuration still uses the existing optional-capacity default. Keeping unsupported evidence on disk does not require the current evaluator to interpret it.

Validate every completed candidate before creating frozen input evidence, schedules or judge calls. Invalid/unsupported completed candidates cannot be silently excluded. Frozen input loading and resume reapply the current contract to the frozen selection; old manifests/caches remain immutable and cannot regain acceptance through legacy labels. Preserve exact source/config/bundle/projection/request/judgment/provenance hashes, original bytes, JSON-type-sensitive comparison and material-bound cache invalidation. No snapshot mutation, migration, regeneration, hash exemption or new mandatory code/provenance fingerprint is permitted.

Independent testing replaces obsolete historical-acceptance expectations with rejection cases for each projection/checkpoint combination except current/current, missing captured capacity, compatibility counters including zero, mixed/partial artifacts, old frozen manifests and stale caches. Use isolated current-format fixtures for positive discovery/freezing/loading/resume and all unaffected evaluator behavior; retain historical fixture bytes only where useful for rejection evidence. Prove rejection before publication/calls with unchanged original evidence, current-format byte/hash preservation, strict nested boolean/number distinction and implementation/input-bound cache invalidation. Coding does not edit tests. Revalidate affected evaluator and K=21–F=25 integration paths before reviewer reapproval.
...
Snapshot validation must enforce the current-only Section 1.6/D12.1.5 projection, checkpoint and captured-effective-configuration requirements while preserving original byte/material bindings. Reject unsupported formats before freezing or calls, without snapshot/prior-evidence rewrites. Cache reuse still requires actual material identity and must reject implementation/input changes.
...
**Historical snapshot boundary.** Historical execution remains evidence of its original producing contract; prefix-only or serial execution is not retrospectively a defect. Section 3.3.9 removes the current evaluator's historical-reader guarantee: require current-format projections, complete supported journal-bearing checkpoints and current captured effective configuration under D12.1.5. Preserve prior bytes, hashes, receipts, reports and optional producing provenance; do not migrate, relabel or delete them. The user intends separate regeneration, which this amendment does not execute or authorize agents to run live. The early evaluator development allowance and fixed review base remain, subject to current-format selected inputs. Earlier Madhi observations certify no Step 27/28 completion.
...
Historical compatibility clauses in this approval record are superseded by the selected Section 3.3.9 removal upon its separate implementation approval. The original delivery amendment and K=21/F=25 approval impact remain recorded.
