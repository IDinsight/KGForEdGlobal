...
**Status:** The original 2026-09-11 D12 amendment, including both evidence-condition corrections, is approved for implementation. Steps 24–25 were independently reapproved at `28d4f1218c71237bc7fda1027a8ac55866bd345f`. The user has now selected an early-start extension: Step 27 coding and subsequent independent offline testing may use a validated, frozen existing Madhi mathematics snapshot while Step 26 continues; the evaluator must support N curricula without curriculum-specific branches or a fixed six-input runtime requirement. The user explicitly approved the synchronized early-start extension for implementation on 2026-09-11, after a second review of the governance changes; the approval is recorded in Section 3.3. D12-S/J/R/B/A remain settled and unchanged. Step 26 completion/reviewer approval, all six reviewed snapshots, authorized live evaluation, complete reports and concern dispositions remain required before Step 27 completion and Step 28. See D12.1.1 and Section 3.3. This edit authorizes no live calls, Git mutations or changes to active runs.
...
- **Development start:** coding may implement the complete generalized evaluator using existing Madhi mathematics LP KG artifacts before all Step 26 pipelines finish. A separate testing task may then author and run independent deterministic tests while Step 26 continues. These offline development activities do not require Step 26 completion approval.
...
- **Execution and completion:** early development does not authorize live evaluation. Live evaluation retains Step 26 reviewer approval, independent deterministic harness validation, an exact user-created evaluator candidate and explicit fixed-input execution authorization as prerequisites. Step 27 remains incomplete until all six project snapshots are reviewer-approved under Step 26 and receive all required evaluation components/reports, with no unresolved execution failure, complete concern dispositions and independent final reviewer approval. Step 28 cannot begin from a Madhi-only development result.
- **Review base and concurrent work:** early Step 27 uses revalidated Step 25 SHA `28d4f1218c71237bc7fda1027a8ac55866bd345f` as its fixed review base through coding, testing, remediation and final review. Account for all later changes, including this approved governance extension, in the candidate range. Later Step 26 approval and exact producer snapshot SHAs are additional input evidence, not an implicit replacement of the evaluator review base. Do not alter, stop or restart active production runs, rewrite their evidence or silently change the code/configuration they consume; any required checkout isolation remains subject to the user's Git authorization.
...
Every curriculum selected for an invocation must appear in each applicable component; Step 27 completion evidence must cover all six project curricula. Sampling must include same/cross/missing-rank cases; repeated or highly overlapping text; absent, shared, nonshared, and broadly reused LC evidence; DAG paths and unresolved ancestry; checker corrections; and truncated/bounded production evidence. Counts must expose absent strata, exhausted strata, and unsupported comparisons. Zero available cases is not evidence of good performance and must not be filled with invented real examples.
...
Step 24 is production D12 release-policy conformance, reassessed under this amendment to distinguish the independent Step 27 obligation. Step 25 remains the structural/process matrix and is revalidated with Step 24. Former Step 27 full runs move to Step 26 without losing any source/config/provenance/count/collision/checkpoint/reuse/D13 checks. Step 27 coding and subsequent independent offline testing may start early under D12.1.1 using the validated frozen Madhi development snapshot, with generalized N-curriculum behavior. Step 26 reviewer approval remains required for live evaluation and final Step 27 completion; the complete required evaluation/reporting evidence still covers all six reviewed project snapshots. Step 28 documents both kinds of evidence; Step 29 reviews them separately.
...
Invariants 1–57 remain production code-level contracts. Invariant 58 preserves structural-only production success while acknowledging the separate amended completion obligation. Invariants 59–66 govern only the Step 27 evaluator. D12.1.1 separates early offline development from live evaluation and final completion, which retain Step 26 reviewer approval. The user's implementation approval of the early-start extension is recorded in Section 3.3.
...
66. **SETTLED — N-curriculum evaluator and staged readiness:** generic evaluation supports any explicitly selected N >= 1 compatible curriculum snapshots without curriculum-name, roster or fixed-count branches. Every selected snapshot must validate; unavailable unselected curricula do not block offline development. A Madhi development snapshot cannot substitute for the six reviewed Step 26 snapshots, authorized evaluation and complete reporting required at the final Step 27 gate.

---

# 5. Build order

No implementation starts at Step 1 until Step 0 is complete. The sole early-start exception is D12.1.1: Step 27 coding and subsequent independent offline testing may overlap Step 26 after approval of that extension; live evaluation and final Step 27 completion retain the Step 26 reviewer gate.
...
| 26   | Run all six complete pipelines from source PDFs (former Step 27)                                 | Six complete result directories and immutable manifests                                                                                                                 | Testing-primary after revalidated Step 25 approval; explicit live authorization. Preserve all AS/AS+LC/AS+LC+LP validation, D13 coverage, source/config/code hashes, provenance, warnings, counts, collisions, projections, checkpoints and reuse obligations. needs_review stays visible/nonpublishing/nonblocking. Step 27 offline development may overlap under D12.1.1; Step 26 reviewer approval remains required before live evaluation and final Step 27 completion.                                                                         |
| 27   | Implement, independently test, and execute the generalized LP evaluation harness                 | entries/evaluate_lps.py; evals/lp_eval/{schemas,sampling,prompts,judge,scoring}.py; separate lp_eval artifacts                                                          | After approval of D12.1.1, coding may start from the revalidated Step 25 base using a validated frozen Madhi snapshot while Step 26 continues. Support any selected N >= 1 curricula without a hard-coded roster or count. Separate testing may validate offline after coding. Live evaluation requires Step 26 reviewer approval and explicit authorization; final completion still requires all six reviewed snapshots, all D12 components/reports/dispositions, exact candidate binding and independent reviewer approval. No production tuning. |
