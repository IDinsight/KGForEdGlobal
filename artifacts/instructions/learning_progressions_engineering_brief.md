...
**Status:** The user explicitly approved the completed synchronized 2026-09-12 concurrency amendment for implementation, including all settled D13-C1/C2/C3 payloads. Approval is recorded in Section 3.3.2. New coding-owned Step 26 precedes renumbered Step 27 full six-pipeline validation; the generalized evaluator, documentation and terminal review become Steps 28, 29 and 30. Steps 0–25 retain their obligations and Step 25 approval at `28d4f1218c71237bc7fda1027a8ac55866bd345f`. D13-C1/C2/C3 are SETTLED: optional production concurrency defaults to 4, shutdown finishes active calls only, and a durable journal preserves validated out-of-order work with strict material-identity compatibility. No implementation-governing policy payload remains open. New Step 26 coding may proceed in a separate coding task, followed by independent testing and reviewer approval. Previously approved early evaluator coding and subsequent independent offline testing remain permitted under D12.1.1 before new Step 26 readiness. Live evaluation and final evaluator completion retain full-pipeline reviewer approval, all six reviewed snapshots, exact candidate/input binding, reporting/dispositions and separate explicit live authorization. Historical approvals remain recorded in Section 3.3.1. This governance task grants no live-call, Git or active-run authorization.
...
Step 26's settled internal pending-completion journal under D13-C3 supplements recovery; existing successful JSONL stays prefix-shaped and public graph/AS/AS+LC contracts remain intact. The user-approved amendment assigns its implementation to new Step 26.

### Separate evaluation artifacts (Step 28)

The evaluator accepts an explicit set of N curriculum snapshots (N >= 1); the six named project curricula are the required completion population, not a hard-coded runtime roster. Early development may select only the validated, frozen Madhi mathematics snapshot under D12.1.1. Every manifest and report identifies its actual selected population and whether it is development evidence; absent future curricula cannot be represented as evaluated or as empty successful strata.
...
The following markers are load-bearing. D1–D11, existing D13, and D14 production semantics remain **SETTLED**; D13-C1/C2/C3 are **SETTLED** and their synchronized amendment was explicitly approved for implementation on 2026-09-12. D12-S/J/R/B/A record the user's settled amendment decisions and complete payloads. The user separately approved the D12 amendment for implementation on 2026-09-11; the concurrency amendment received its own separate implementation approval on 2026-09-12. Ordinary role, build-order, reviewer and live-execution gates still apply.
...
| D13-C3 | Durable out-of-order completion and compatibility                   | Sole-writer durable completion journal; contiguous successful prefixes; strict material-identity compatibility                                                                        | **SETTLED** |
| D14    | Manual semantic edge overrides in v1                                | A — no forced semantic include/exclude/relation/direction overrides                                                                                                                   | **SETTLED** |

Settled policy text is implementation-governing after its required explicit implementation approval. D13.1 policy selection and the separate amendment implementation approval are recorded in Section 3.3.2. Rejected alternative analysis is historical only and cannot govern implementation unless a later user-approved governance change reopens that decision.
...
- **Development input integrity:** before consuming the Madhi artifacts, identify and validate a complete frozen snapshot with exact production code SHA, source identity, effective configuration, AS+LC and LP bundles/projections, original bounded requests, producer/checker/final judgments, provenance, validation reports and material hashes. Preserve the original evidence required by D12.3. An existing final graph alone is insufficient. Record actual paths, hashes and validation status in a development manifest; this decision does not certify any directory as complete or reviewer-approved. Never consume a growing run or invent missing material.
- **Generalized runtime:** accept any explicitly selected positive number N of compatible curriculum snapshots, including one, several or more than six. Derive identity, paths, local labels/order, policy permissions, evidence limits, populations and reporting groups from supplied snapshots/configuration. Generic evaluator code, prompts, schemas, sampling and scoring must not hard-code Madhi, another curriculum, an enumerated roster, a fixed count of six, curriculum-specific statement types, grade values, hierarchy shapes or evidence semantics. Keep populations framework-contained; no cross-framework pairs. Adding a compatible future snapshot requires input/configuration, not a code change.
- **Incremental availability:** unselected curricula whose outputs are not yet available do not block development or preparation for the selected set. A selected missing, incomplete, stale or invalid snapshot fails validation; do not silently drop it, fabricate examples or label it an exhausted stratum. Later curricula enter through new explicit manifests and schedules. Preserve earlier evidence and reuse only fully validated, materially identical requests under D12.4. Never relabel a one-curriculum report as complete six-curriculum evidence.
...
Concurrent/out-of-order completions must be buffered or otherwise serialized without reusable prefix gaps. New Step 26 adds D13.1's bounded dispatch, durable pending-completion and shutdown behavior only after explicit implementation approval of the settled amendment.
...
### D13.1 Settled Step 26 concurrency contract — approved for implementation

Step 26 owns this new capability; historical Steps 12–15 and 22–25 were not required to dispatch concurrently. Their existing materialization, integrity, checkpoint and failure obligations remain binding. The user selected the complete operational/compatibility payloads below on 2026-09-12. The user subsequently approved the completed synchronized amendment for implementation, as recorded in Section 3.3.2.
...
**SETTLED D13-C3 — Durable completion journal and strict material compatibility**

Add an internal pending-completion journal controlled by the sole writer, separate from successful JSONL and failures, integrated with the existing exclusive directory ownership and crash-safe transaction/receipt mechanism. Retain validated stage payloads, request index/ID, exact producer/checker dependencies, material hashes and attempt/usage identities. Gaps may exist only in the pending journal, never in successful JSONL. Reject duplicates, corruption, stale identity and inconsistent dependencies. Recovery/prefix promotion is idempotent without duplicate calls or accounting; validate journal records and their transaction/prefix state before any reuse. Coding owns the internal journal path/schema names, not a new compatibility-policy choice. No public graph schema change or runtime checkpoint-policy selector is authorized. Persist completed work before releasing its capacity slot, with bounded in-memory dispatch/buffering.

Existing valid prefix-only checkpoints with identical actual material identities remain reusable without loss; no journal means no pending results, not invented success. If effective config changes—including a newly introduced/defaulted concurrency field that changes captured identity—fail closed, preserve old evidence and require separately authorized regeneration of affected material before calls. Do not silently normalize/migrate old hashes. The same-effective-value rule in C1 does not permit retrofitting that value into legacy receipts to make mismatched evidence reusable. Existing frozen completed snapshots remain eligible for early evaluator development when D12 validation passes; that does not certify them as post-concurrency Step 27 execution evidence. This governance task modifies no run directory.

Separate testing independently challenges these settled payloads and all properties after amendment implementation approval; reviewer approval follows coding, independent testing and the exact user-created candidate. No dependent production/config/environment/test implementation begins before that explicit implementation approval.
...
Section 3.3.2 governs the current amendment. Historical approvals and next-step handoffs below retain their original numbering and do not override the current order.
...
66. **SETTLED — N-curriculum evaluator and staged readiness:** generic evaluation supports any explicitly selected N >= 1 compatible curriculum snapshots without curriculum-name, roster or fixed-count branches. Every selected snapshot must validate; unavailable unselected curricula do not block offline development. A Madhi development snapshot cannot substitute for the six reviewed Step 27 snapshots, authorized evaluation and complete reporting required at the final Step 28 gate.

## 4.8 Settled prospective concurrency invariants (Step 26 only; approved for implementation)

Under the recorded implementation approval, Step 26 must enforce settled D13.1: optional positive-integer capacity defaulting to 4, bounded admission/buffers, per-request validated producer-before-checker order, sole-writer crash-safe journal retention and contiguous prefix promotion, strict material-bound stage reuse, active-calls-only shutdown and complete race-safe accounting. These add concurrency obligations without renumbering/weakening invariants 1–66 or changing production success semantics. Policy selection does not substitute for implementation approval.

---

# 5. Build order

No implementation starts at Step 1 until Step 0 is complete. The sole early-start exception is approved D12.1.1: Step 28 coding and subsequent independent offline testing may proceed while new Step 26 is pending and overlap Step 27. Live evaluation/final Step 28 completion retain Step 27 reviewer approval. New Step 26 has the required amendment implementation approval recorded in Section 3.3.2; Step 27 under the new order requires Step 26 reviewer approval. Existing active runs remain protected under Section 3.3.2.
...
| 26   | Implement bounded concurrency across independent production LP requests                                             | kgs/lp_generation.py; kgs/lp_checkpoints.py; required schema/usage/receipt integration                                                                                  | Coding after explicit implementation approval of settled D13-C1/C2/C3, then independent offline testing and reviewer approval at an exact user-created candidate. Fixed base is revalidated Step 25. Preserve pre-call materialization, per-request stage dependencies, bounded dispatch/buffers, durable contiguous prefixes, completed work, active-calls-only shutdown, strict material identity and accounting; optional capacity defaults to 4. See Section 5.4. No semantic, prompt, candidate or model tuning.                                                               |
...
After new Step 26 reviewer approval, record the effective production concurrency value, exact reviewed candidate and actual material inputs in the Step 27 execution evidence. Check the selected shutdown/reuse/accounting contract wherever exercised. Previously started runs retain their original bindings under Section 3.3.2 and are not silently converted to new-candidate evidence.
...
Coding owns only the approved bounded execution enhancement and minimum schema/checkpoint/usage integration. Separate testing independently derives an offline matrix with controllable barriers/fakes, not live APIs or timing-dependent sleeps:
...
- Interrupt journal writes, prefix promotion and receipt/transaction updates. Recover idempotently without duplicate calls/usage; reuse a saved producer with unfinished checker and completed suffix stages after filling the hole.
...
- Settled legacy/journal compatibility and changed-capacity identity behavior: omitted/explicit 4 resolve identically but raw config evidence stays accurate; changed effective capacity and incompatible legacy identity fail closed. Preserve valid prefix-only evidence and reject silent hash migration.
