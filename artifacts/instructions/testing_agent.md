...
Tell the user to create a candidate review commit when material repository content changed. If the exact repository tree is already a reviewed candidate and only evidence changed, reuse the same candidate SHA rather than requesting a no-op commit.
