...
Verify D12.1.3 permits otherwise-valid snapshots with an unknown producing Git SHA throughout Step 28, preserves any genuinely recorded SHA without fabricating it, and still rejects missing or changed required material inputs and invalid evaluator candidate identities. Do not introduce a replacement producing-code fingerprint prerequisite.
...
Handoff the exact evaluator candidate SHA, tests, production snapshot material hashes and any known producing SHAs, evaluator config and schedule hashes, commands, usage/cost/failure evidence, artifact directory/checksums, concern dispositions and independent test results to the reviewer. A user-created material candidate commit precedes live evidence claimed against that exact SHA; earlier working-tree smoke evidence must be identified as such and is not silently relabeled. Reuse identical candidate SHAs for evidence-only reruns.
