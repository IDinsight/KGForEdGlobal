...
Independent offline tests may begin after coding completion, the recorded settled D12-S/J/R/B/A policies and explicit approval of the early-start extension (recorded in brief Section 3.3 on 2026-09-11), without waiting for Step 26 completion. Use the fixed early Step 27 review base from D12.1.1. Live evaluation and final Step 27 completion still require Step 26 reviewer approval. Independently derive tests for all D12 components and invariants 59–66; the coding handoff and the LC evaluator are not oracles. Executable evaluator source/config/prompts are read-only to testing.

Use deterministic fakes and independent reduced synthetic fixtures to challenge: label/rationale leakage into blind classification; critique contamination; separate support/grounding outcomes; upstream sampling dependence on nominations; outcome/correction/rank/text/LC/DAG/unresolved/truncation strata; empty/exhausted cells and cohort overlap; reproducibility and sampling probabilities; order remapping and repeated-call identities; exact/reconstructed/expanded/removed evidence conditions; synthetic-control expectations and real-pair nonnegative assumptions; baselines; stale/tampered/gapped/duplicate/extra cache outputs; malformed schema/IDs/endpoints/evidence references; interrupted resume; finite per-judgment retries and usage accounting without aggregate budget gates; output/input aliasing; incomplete reports; disagreement/ambiguity preservation; and denominator-zero behavior. Verify production artifacts and success semantics remain unchanged. Test explicit selection of one, multiple and more-than-six synthetic curriculum snapshots, unfamiliar names/local labels, per-framework isolation, and addition of a compatible snapshot without generic code changes. Challenge hard-coded Madhi/six-profile assumptions. Unselected unavailable curricula must not block preparation; a selected missing/stale/invalid snapshot must fail rather than disappear. Distinguish development/subset reports from complete project evidence.
