...
Generated artifacts are evidence of what a particular run produced under particular source/config/model inputs and content hashes. They are not universal product semantics. Explain historical validity under the original producing contract separately from current input eligibility: D12.1.5 proposes strict historical/current readers under Section 3.3.10's recorded implementation approval. Distinguish the currently implemented rejection policy from the proposed six-cell contract; acceptance still requires actual validation without changing prior evidence. An absent exact producing SHA stays unknown; neither successful reconstruction nor an inspection-code SHA supplies it. The supplied Madhi observations in brief Section 3.3.3 are not Step 27/28 approval or concurrency execution evidence.
...
Do not invent final field names from Section 2.13's settled semantic contract. D1–D11, existing D13 and D14 remain settled; D13-C1/C2/C3 are settled and the synchronized amendment received explicit implementation approval on 2026-09-12; the D12 amendment has complete settled policies and received explicit user implementation approval on 2026-09-11. Step 3 owns the original production `kgs.lp` field names; new Step 26 adds the exact optional `max_concurrent_requests` field with default 4 under D13-C1; Step 28 evaluation config remains separate.
...
Distinguish the amended Section 1.6 delivery contract from observed implementation and historical artifacts. The two AS+LC+LP JSONL files preserve AS+LC wire records and add only LP wire relationships, using explicit aliases rather than recursive camelization; the combined bundle and other internal artifacts retain complete internal metadata. D12.1.5 proposes independent historical snake_case, converted-flat camelCase and current wire readers crossed with historical prefixes or current journals. Explain the explicit endpoint-key/top-level mapping, preserved nested metadata and raw/material/interpretation binding; format support is not production checkpoint compatibility. Section 3.3.10 approval is recorded; distinguish the approved contract from actual implementation progress. Distinguish recorded specification approval from observed implementation; do not claim implementation without evidence, infer later approval from HEAD, or suggest rewriting historical results to match current delivery.
...
For new Step 26 explain bounded request admission separately from request batching and evaluator concurrency. Each checker waits for its own validated draft. Settled D13-C1 defaults the optional capacity field to 4 per run. D13-C2 finishes only calls active when exhausted failure is observed, saves valid results and permits no subsequent producer/checker/retry calls. D13-C3 retains out-of-order work in a durable journal while successful JSONL remains contiguous, with strict material-identity checks. These policies and the synchronized amendment are approved for implementation, as recorded in Section 3.3.2. Do not describe the capability as implemented. Early Step 28 development remains allowed before new Step 26 approval and is separate from final live readiness.
...
The amended order adds prospective coding-owned Step 26 concurrency after revalidated Steps 24–25, then Step 27 six complete pipelines after Step 26 reviewer approval. The user approved the early-start extension on 2026-09-11, as recorded in brief Section 3.3; D12.1.1 allows Step 28 coding and subsequent independent offline testing to overlap Step 27 using a validated frozen Madhi development snapshot. The harness is generalized for any N >= 1 compatible curricula selected by D12.1.2 recursive discovery and frozen manifests; the initial six are a project-completion population, not a runtime roster/count. Missing unselected future outputs do not block development, but selected invalid/missing inputs fail. Live evaluation and final Step 28 approval still require Step 27 reviewer approval and all six reviewed snapshots, complete evaluation/reporting and concern disposition. Step 29 documentation and Step 30 comprehensive review follow; a Madhi-only development result is not Step 28 completion. The global no-required-sample/metric rule changed at Step 24, so the user's adoption on 2026-09-11 reopened Steps 24–25 (subsequently reapproved as recorded in Section 3.3.1) at review base `540ea950378ce54b54da1c7a93491b525609b574`; distinguish the recorded user specification approval from independent reviewer approval of code or steps, and do not imply it authorizes production tuning, live calls or bypassing progression gates.
